...
from silver_screen.voice_config import (
    VoiceStudioError,
    build_voice_cast,
    build_voice_plan,
...
from silver_screen.voice_studio import (
    _ffmpeg_path,
    merge_voice_result,
...
def _state() -> dict[str, Any]:
    return {
        ...
        "seed": 42,
        "characters": [
            {"id": "moonie", "name": "Moonie Moo", "role": "Celebrity cow"},
            {"id": "bully", "name": "Bully", "role": "Loyal bulldog"},
        ],
        "scenes": [
            {
                "number": 1,
                "summary": "Moonie steps onto the red carpet as Bully steals the cameras.",
                "shots": [
                    {
                        "dialogue": 'Moonie Moo: "Darling, the spotlight always finds me."'
                    },
                    {"dialogue": 'Bully: "Not if I find it first."'},
                ],
            }
        ],
    }
...
def _video_result(
    root: Path, count: int = 2, status: str = "complete"
) -> dict[str, Any]:
    shots = []
        ...
        shots.append(
            {
                ...
                "plannedDurationSeconds": 1.2,
                "verifiedDurationSeconds": 1.2,
            }
        )
    final = _make_video(
        root
        / (
            "final_ai_film.mp4"
            if status == "complete"
            else "partial_ai_film.mp4"
        ),
        seconds=max(1.2, count * 1.2),
    )
    return {
        "status": status,
        "mode": "ai-video",
        "queue": {"productionId": "video_test", "shots": shots},
        "metrics": {
            "plannedShots": count,
            "verifiedShots": count,
            "verifiedSeconds": count * 1.2,
        },
        "final_video_path": str(final) if status == "complete" else None,
        "partial_video_path": str(final) if status != "complete" else None,
        "warnings": [],
    }
...
def test_cast_and_plan_are_deterministic(tmp_path: Path) -> None:
    ...
    assert [line["signature"] for line in first["lines"]] == [
        line["signature"] for line in second["lines"]
    ]
...
def test_uploaded_voice_requires_authorization(tmp_path: Path) -> None:
    ...
    with pytest.raises(VoiceStudioError, match="authorization"):
        prepare_voice_config(
            tmp_path,
            [{**_request(custom_voice=True), "voice_sample": str(sample)}],
        )


def test_elevenlabs_voice_ids_require_authorization(
    ...
    config = normalize_voice_config(
        _request(
            provider="elevenlabs",
            lead_voice="lead-id",
            supporting_voice="support-id",
            narrator_voice="narrator-id",
            authorization_confirmed=False,
        )
    )
    with pytest.raises(VoiceStudioError, match="authorized"):
        validate_voice_config(config)


def test_voice_generation_repairs_and_resumes_without_duplicate_calls(
    ...
    resumed = process_voice_production(
        _state(),
        video,
        tmp_path,
        voice_inputs=None,
    ...
    result = process_voice_production(
        ...
        voice_inputs=[
            _request(
                ...
                manual_tracks=[str(manual_a), str(manual_b)],
            )
        ],
    )
    assert result["status"] == "complete"
...
def test_merge_preserves_original_silent_film(tmp_path: Path) -> None:
    ...
    merged = merge_voice_result(
        {
            "final_video_path": str(silent),
            "partial_video_path": None,
            "hero_path": str(silent),
            "warnings": [],
        },
        {
            "status": "complete",
            "final_video_path": str(voiced),
            "partial_video_path": None,
            "warnings": [],
        },
    )
...
def test_voice_diagnostics_are_actionable() -> None:
    ...
    assert billing.code == "billing_required"
