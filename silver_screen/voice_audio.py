...
from .voice_config import AUDIO_SUFFIXES, VoiceStudioError, _ffmpeg_path, _ffprobe_path, _relative, _safe_path

def verify_audio(path: Path) -> dict[str, Any]:
    if not path.is_file() or path.stat().st_size < 512:
        raise VoiceStudioError(f'Generated audio is missing or too small: {path}')
    metadata: dict[str, Any] = {'bytes': path.stat().st_size, 'durationSeconds': None}
    ffprobe = _ffprobe_path()
    if not ffprobe:
        return metadata
    completed = subprocess.run([ffprobe, '-v', 'error', '-show_entries', 'format=duration', '-of', 'json', str(path)], capture_output=True, text=True, timeout=30)
...
def _has_audio(path: Path) -> bool:
    ...
    if not ffprobe:
        return False
    completed = subprocess.run([ffprobe, '-v', 'error', '-select_streams', 'a', '-show_entries', 'stream=index', '-of', 'csv=p=0', str(path)], capture_output=True, text=True, timeout=30)
    return completed.returncode == 0 and bool(completed.stdout.strip())

def mix_voice_into_clip(video_path: Path, audio_path: Path, destination: Path, *, duration: float, preserve_source_audio: bool, delay_seconds: float=0.2) -> Path:
    ffmpeg = _ffmpeg_path()
    if not ffmpeg:
    ...
    voice_filter = f'[1:a]aresample=48000,adelay={delay_ms}:all=1,apad,atrim=0:{duration:.3f},volume=1.15[voice]'
    command = [ffmpeg, '-y', '-i', str(video_path), '-i', str(audio_path)]
    if preserve_source_audio and _has_audio(video_path):
        graph = f'[0:a]aresample=48000,volume=0.22,apad,atrim=0:{duration:.3f}[base];{voice_filter};[base][voice]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[aout]'
    else:
        graph = voice_filter.replace('[voice]', '[aout]')
    command.extend(['-filter_complex', graph, '-map', '0:v:0', '-map', '[aout]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-ar', '48000', '-t', f'{duration:.3f}', '-movflags', '+faststart', str(destination)])
...
def _video_shots(video_result: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    return {str(shot.get('id') or ''): shot for shot in queue.get('shots') or [] if isinstance(shot, dict)}
...
def _reconcile(root: Path, plan: dict[str, Any], verified_video: dict[str, dict[str, Any]]) -> None:
    for line in plan.get('lines') or []:
        ...
        try:
            ...
            verify_mp4(_safe_path(root, line['dubbedPath']), expected_duration=float(line.get('targetDurationSeconds', 0) or 0))
        except Exception as exc:
            line.update({'status': 'pending', 'lastError': f'Reconciliation: {exc}', 'audioPath': None, 'dubbedPath': None, 'audioDurationSeconds': None, 'completedAt': None})

def _manual_track(root: Path, config: dict[str, Any], line: dict[str, Any], eligible_index: int) -> Path:
    paths = [_safe_path(root, value) for value in config.get('manual_track_paths') or []]
...
def _repair_line(line: dict[str, Any], diagnosis_code: str, attempt: int) -> dict[str, Any]:
    repair = {'attempt': attempt, 'code': diagnosis_code, 'at': utc_now(), 'strategy': 'retry_same_line'}
    if diagnosis_code in {'invalid_audio', 'unknown'}:
        repair['strategy'] = 'shorten_and_simplify'
        words = str(line.get('text') or '').split()
        if len(words) > 7:
            line['text'] = ' '.join(words[:max(7, math.ceil(len(words) * 0.75))])
        line['instructionsOverride'] = 'Speak clearly and naturally without sound effects.'
...
def _reinforce(plan: dict[str, Any], line: dict[str, Any], repair: dict[str, Any]) -> None:
    ...
    for scar in scars:
        if isinstance(scar, dict) and scar.get('key') == key:
            ...
            return
    scars.append({'key': key, 'strategy': repair.get('strategy'), 'speaker': line.get('speaker'), 'provider': line.get('provider'), 'uses': 1, 'createdAt': utc_now(), 'lastUsedAt': utc_now()})
...
def render_subtitles(plan: dict[str, Any]) -> str:
    ...
    blocks: list[str] = []
    index = 1
    for line in sorted([item for item in plan.get('lines') or [] if isinstance(item, dict)], key=lambda item: int(item.get('order', 0) or 0)):
        ...
        if line.get('status') == 'verified' and line.get('text'):
            ...
            blocks.append('\n'.join([str(index), f'{_srt_time(start)} --> {_srt_time(end)}', f"{prefix}{line.get('text')}"]))
            index += 1
        cursor += duration
    return '\n\n'.join(blocks) + ('\n' if blocks else '')
