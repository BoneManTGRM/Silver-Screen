...
run artifacts. The standard-library HTTP implementation keeps the production
container independent from provider SDK versions.
"""
...
class VoiceProviderError(RuntimeError):
    """Raised when a voice provider cannot create usable audio."""
...
def diagnose_voice_error(error: str) -> VoiceErrorDiagnosis:
    text = (error or "").lower()
    if any(token in text for token in ("401", "unauthorized", "invalid api key", "incorrect api key")):
    ...
    if any(token in text for token in ("billing", "credit", "quota", "payment", "insufficient_quota")):
        return VoiceErrorDiagnosis(
            "billing",
            "Voice-provider billing or credits are required",
            "Add usable provider credits or choose manual uploaded audio.",
            False,
        )
    if any(token in text for token in ("consent", "permission", "not authorized", "forbidden", "403")):
        return VoiceErrorDiagnosis(
            ...
            "Use an authorized voice, provide genuine provider consent, or select a built-in voice.",
            False,
        )
    if any(token in text for token in ("voice_id", "voice id", "voice not found", "404")):
        return VoiceErrorDiagnosis(
            "voice_not_found",
    ...
    if any(token in text for token in ("400", "invalid request", "invalid input", "unprocessable")):
        return VoiceErrorDiagnosis(
            "invalid_request",
            "The voice provider rejected the request",
            "Check the voice ID, model, text, uploaded audio format, and consent fields.",
            False,
        )
    if any(token in text for token in ("429", "rate limit", "too many requests")):
        return VoiceErrorDiagnosis(
            ...
            "Keep the saved checkpoint and retry the same line after the limit resets.",
            True,
        )
    if any(token in text for token in ("timeout", "timed out", "temporarily unavailable", "502", "503", "504")):
        return VoiceErrorDiagnosis(
            ...
            "Retry the same saved line instead of creating a duplicate production.",
            True,
        )
    if any(token in text for token in ("safety", "policy", "moderation")):
        return VoiceErrorDiagnosis(
            ...
            "Revise the affected line and use only authorized, non-deceptive content.",
            False,
        )
    if any(token in text for token in ("audio", "mp3", "ffprobe", "empty", "too small", "decode")):
        return VoiceErrorDiagnosis(
            "invalid_audio",
            "The provider response was not usable audio",
            "TGRM can retry only the affected line with simplified delivery settings.",
            True,
        )
    return VoiceErrorDiagnosis(
        "unknown",
...
def provider_capabilities() -> dict[str, Any]:
    return {
        ...
        "manual": True,
        "openaiModel": os.getenv("SILVER_SCREEN_OPENAI_TTS_MODEL", "gpt-4o-mini-tts"),
        "elevenLabsModel": os.getenv("SILVER_SCREEN_ELEVENLABS_MODEL", "eleven_multilingual_v2"),
...
def _multipart_body(fields: dict[str, str], files: dict[str, Path]) -> tuple[bytes, str]:
    ...
    for name, path in files.items():
        if not path.exists() or not path.is_file():
            raise VoiceProviderError(f"Required voice file is missing: {path}")
...
class OpenAIVoiceProvider:
    ...
    def __init__(self, token: str | None = None, model: str | None = None) -> None:
        ...
        self.model = (
            model or os.getenv("SILVER_SCREEN_OPENAI_TTS_MODEL") or "gpt-4o-mini-tts"
        ).strip()
        if not self.token:
    ...
    def synthesize(
        ...
        *,
        ...
        if "json" in content_type.lower():
            detail = body.decode("utf-8", errors="replace")[:1200]
            raise VoiceProviderError(f"OpenAI returned JSON instead of audio: {detail}")
    ...
    def create_consent(self, *, recording: Path, name: str, language: str = "en-US") -> str:
        payload = _multipart_request(
            ...
            fields={"name": name[:120], "language": language[:35]},
    ...
    def create_custom_voice(self, *, audio_sample: Path, consent_id: str, name: str) -> str:
        payload = _multipart_request(
            ...
            headers={"Authorization": f"Bearer {self.token}"},
            fields={"name": name[:120], "consent": consent_id},
...
class ElevenLabsVoiceProvider:
    ...
    def __init__(self, token: str | None = None, model: str | None = None) -> None:
        ...
        self.model = (
            model or os.getenv("SILVER_SCREEN_ELEVENLABS_MODEL") or "eleven_multilingual_v2"
        ).strip()
        if not self.token:
    ...
    def synthesize(
        ...
        *,
        ...
        if "json" in content_type.lower():
            detail = body.decode("utf-8", errors="replace")[:1200]
            raise VoiceProviderError(f"ElevenLabs returned JSON instead of audio: {detail}")
        return _atomic_bytes(destination, body)
