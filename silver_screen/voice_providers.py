...
def diagnose_voice_error(error: str) -> VoiceProviderDiagnosis:
    ...
    if any(token in text for token in ("401", "unauthorized", "invalid api key", "incorrect api key", "authentication")):
        return VoiceProviderDiagnosis(
            "authentication",
            "The voice-provider key was rejected",
            "Check the configured API key and redeploy the app before retrying.",
            False,
        )
    if any(token in text for token in ("billing", "payment", "credit", "quota", "insufficient_quota")):
        return VoiceProviderDiagnosis(
            ...
            "Add usable provider credits or use authorized manual audio tracks.",
            False,
        )
    if any(token in text for token in ("consent", "permission", "not authorized", "forbidden", "403")):
        return VoiceProviderDiagnosis(
            "consent",
            "The selected voice lacks valid authorization or consent",
            "Use an authorized voice, upload the provider-required consent recording, or select a built-in voice.",
            False,
        )
    ...
    if any(token in text for token in ("timeout", "timed out", "temporarily unavailable", "502", "503", "504")):
        return VoiceProviderDiagnosis(
            ...
            "Retry the same saved line rather than creating a duplicate production.",
            True,
        )
    if any(token in text for token in ("safety", "policy", "moderation")):
        return VoiceProviderDiagnosis(
            ...
            "Revise only the affected line and use authorized, non-deceptive content.",
            False,
        )
    ...
    if any(token in text for token in ("could not reach", "connection", "network", "dns", "urlopen")):
        return VoiceProviderDiagnosis(
            ...
            "Keep the saved checkpoint and retry after network access is restored.",
            True,
        )
    if any(token in text for token in ("400", "invalid request", "invalid input", "unprocessable", "422")):
        return VoiceProviderDiagnosis(
            ...
            "Check the model, voice ID, text, audio format, and consent fields before retrying.",
            False,
        )
    return VoiceProviderDiagnosis(
        "unknown",
        "Voice generation failed",
        "Inspect the exact error and retry only the affected spoken line.",
        True,
    )
...
class OpenAITTSProvider:
    ...
    def synthesize(
        ...
        *,
        ...
        if "json" in content_type.lower():
            ...
            raise VoiceProviderError(
                f"OpenAI returned JSON instead of audio: {detail[:1200]}"
            )
        return _atomic_write_bytes(destination, body)

    def create_consent(
        self, *, recording: Path, name: str, language: str = "en-US"
    ) -> str:
        payload = _multipart_request(
            url="https://api.openai.com/v1/audio/voice_consents",
            token=self.token,
            fields={"name": str(name)[:120], "language": str(language)[:35]},
    ...
    def create_custom_voice(
        self, *, audio_sample: Path, consent_id: str, name: str
    ) -> str:
        payload = _multipart_request(
            url="https://api.openai.com/v1/audio/voices",
            token=self.token,
            fields={"name": str(name)[:120], "consent": str(consent_id)},
    ...
    def synthesize(
        ...
        *,
        ...
        if "json" in content_type.lower():
            detail = body.decode("utf-8", errors="replace")
            raise VoiceProviderError(
                f"ElevenLabs returned JSON instead of audio: {detail[:1200]}"
            )
        return _atomic_write_bytes(destination, body)
