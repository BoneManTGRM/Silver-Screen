...
def _paths(root: Path) -> dict[str, Path]:
    ...
    return {
        "root": audio,
        "config": audio / VOICE_CONFIG_FILENAME,
        "cast": audio / VOICE_CAST_FILENAME,
        "plan": audio / VOICE_PLAN_FILENAME,
        "runtime": audio / VOICE_RUNTIME_FILENAME,
        "scars": audio / VOICE_SCARS_FILENAME,
        "subtitles": audio / SUBTITLES_FILENAME,
        "inputs": audio / "inputs",
        "lines": audio / "lines",
...
def normalize_voice_config(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    config = {
        ...
        "authorization_confirmed": bool(
            source.get("authorization_confirmed", False)
        ),
        ...
        "consent_recording_path": str(
            source.get("consent_recording_path") or ""
        ),
        "manual_track_paths": [
            str(item) for item in source.get("manual_track_paths") or [] if item
        ],
        "allow_original_fallback": bool(
            source.get("allow_original_fallback", True)
        ),
        "line_delay_seconds": max(
            0.0,
            min(2.0, float(source.get("line_delay_seconds", 0.2) or 0.0)),
        ),
    }
    if provider == "openai":
        for key, fallback in (
...
def prepare_voice_config(
    ...
    if (sample is not None or consent is not None or tracks) and not config[
        "authorization_confirmed"
    ]:
        raise VoiceStudioError(
            "Uploaded or custom voice material requires explicit authorization confirmation"
        )
    ...
    for index, upload in enumerate(tracks, start=1):
        ...
        path = _persist_upload(
            upload, paths["inputs"] / "manual" / f"{index:04d}-{filename}"
        )
...
def validate_voice_config(config: dict[str, Any]) -> None:
    ...
    if provider == "elevenlabs":
        if not config.get("authorization_confirmed"):
            raise VoiceStudioError(
                "ElevenLabs voice IDs require confirmation that the selected voices are authorized"
            )
        missing = [
            key
        ...
        if missing:
            raise VoiceStudioError(
                "ElevenLabs requires voice IDs for: " + ", ".join(missing)
            )
    ...
    if provider == "manual" and not config.get("authorization_confirmed"):
        raise VoiceStudioError(
            "Manual voice tracks require confirmation that the recordings are authorized"
        )
    if config.get("custom_voice"):
        ...
        if not config.get("authorization_confirmed"):
            raise VoiceStudioError(
                "Custom voice enrollment requires authorization confirmation"
            )
...
def _voice_for_speaker(cast: dict[str, Any], speaker: str) -> str:
    for item in cast.get("characters") or []:
        if (
            ...
            and str(item.get("character") or "").casefold() == speaker.casefold()
        ):
...
def build_voice_plan(
    ...
    for shot in shots:
        ...
        reusable = bool(
            ...
            and old.get("dubbedPath")
        )
        lines.append(
            {
                "id": f"voice_{line_key}",
                "shotId": shot_id,
                "order": order,
                "scene": scene_number,
                "speaker": speaker,
                ...
                "attempts": int(old.get("attempts", 0) or 0) if reusable else 0,
                "audioPath": old.get("audioPath") if reusable else None,
                "dubbedPath": old.get("dubbedPath") if reusable else None,
                "audioDurationSeconds": (
                    old.get("audioDurationSeconds") if reusable else None
                ),
                "lastError": old.get("lastError") if reusable else None,
                "repairs": list(old.get("repairs") or []) if reusable else [],
                "createdAt": old.get("createdAt") or utc_now(),
...
def _update_metrics(plan: dict[str, Any]) -> None:
    lines = [
        line for line in plan.get("lines") or [] if isinstance(line, dict)
    ]
    eligible = [
        line
        for line in lines
    ...
    seconds = round(
        sum(float(line.get("audioDurationSeconds", 0) or 0) for line in completed),
        3,
    )
