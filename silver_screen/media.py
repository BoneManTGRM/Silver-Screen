"""Media rendering for cards, previews, resumable AI films, and voices."""
...
def _make_card(
    ...
    draw.text(
        (70, 68),
        str(state.get("title") or "Silver-Screen")[:80],
        font=_font(56, True),
        fill=accent,
    )
    draw.text(
        (72, 160),
        str(chapter.get("title") or "Chapter")[:100],
        font=_font(38, True),
        fill=(255, 255, 255),
    )
    _draw_wrapped(
...
def _voice_requested(voice_inputs: list[Any]) -> bool:
    return any(
    ...
    *,
    ...
    if mode == "ai-video":
        result = generate_ai_video(
            ...
            max_provider_calls=video_max_provider_calls,
            max_spend_usd=video_max_spend_usd,
            cost_per_second_usd=video_cost_per_second_usd,
